...
/* ── queue ───────────────────────────────────────────────────────────────── */
uint32_t queue_create(uint32_t item_size, uint32_t num_items)
{
    if (port_is_privileged()) {
        queue_t *q = z_impl_queue_create(item_size, num_items);
        if (!q) return KOBJ_INVALID;
        return k_obj_alloc(q, KOBJ_QUEUE);
    }
    return (uint32_t)port_syscall_invoke(SYSCALL_QUEUE_CREATE,
                                         (unsigned int)item_size,
                                         (unsigned int)num_items, 0, 0);
}
...
uint32_t sem_create(int initial_count)
{
    ...
    return (uint32_t)port_syscall_invoke(SYSCALL_SEM_CREATE,
                                         (unsigned int)(uint32_t)initial_count,
                                         0, 0, 0);
}
